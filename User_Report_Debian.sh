#!/bin/sh
#   Here the script,  you can save it make it executable then run it trough less :  User_report.sh  |  less
#   or save the output with:  User_report.sh  > Report_$(hostname )_$(date +%F)
#   User_report.sh  > Report_$(hostname)_$(date +%F) && gzip Report_$(hostname)_$(date +%F)

   #!/bin/sh
   # Debian Workstation user report
   #
   # Section dividers
   DEV1=" # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - #\n #############################################################\n"
   DEV2=" #############################################################\n # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - #"
   ...
   FL='\n\n\n\n'
   #
   printf "\t####  Debian  OS  &  WORKSTATION  INFO  ####\n" 
   printf "\n\t\t$(date)\n\n"
   printf "$LIN1"
   ...
   printf "$LIN2"
   printf "$LIN1"
   printf ' Debian - uname command - Information about the system'
   printf "$LIN1"
   ...
   echo ' - Debian version of the user environment : ' $(uname -U)
...
#   In order to save the script read Revadig.com howto :  Dump a Web page formatted text and edit in Vim
#   In links avoid broken lines with the  "-width"  dumping option, e.g.:
#   links   -width 300  -dump   https://revadig.blogspot.com/2019/09/freebsd-workstation-user-report.html
